...
| 🌱 Project Workflows | Project-scoped seed intake, frozen manifest templates, and formal seed-lot weight ledgers |
| ✅ Tasks | Lab task discovery/execution, scheduled stages, assigned staff stages, and platform-admin global task management |
| 🧰 Tool Catalog | Discover, validate, submit, rerun, and review published reproducible tools |
...
| `scitex-tashan` | Tashan (他山) project seed intake, frozen manifests, record workflows, and the SeedLot weight ledger |
...
| `scitex-tool` | Published Tool Catalog discovery, validation, confirmed execution, exact reruns, and bounded artifact previews |
...
| 🌱 项目工作流 | 按 slug 进行种子入库、冻结清单模板与正式 SeedLot 重量账本操作 |
| ✅ 任务 | Lab 任务定义检索与执行、定时阶段、员工待办阶段、平台管理员全局任务管理 |
| 🧰 Tool Catalog | 检索、校验、提交、精确重跑和审阅已发布的可复现工具 |
...
| `scitex-tashan` | 他山项目的种子入库、冻结清单、记录流程与 SeedLot 重量账本 |
| `scitex-task` | 自然语言创建任务/工作流：先搜索课题组任务类型摘要、读取选中详情，再创建单阶段或多阶段任务 |
| `scitex-tool` | 已发布 Tool Catalog 的检索、校验、确认执行、精确重跑和有限 artifact 预览 |
